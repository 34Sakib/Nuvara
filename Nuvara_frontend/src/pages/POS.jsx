...
export const POS = () => {
  ...
  const [held, setHeld] = useState(() => { try { return JSON.parse(localStorage.getItem('nuvara_held_sale') || 'null'); } catch { return null; } });

  useEffect(() => {
    let active = true;
    api.get('/products', { params: { limit: 100 } }).then(({ data }) => {
      if (active && Array.isArray(data.data) && data.data.length) setCatalog(data.data);
    }).catch(() => {}).finally(() => { if (active) setCatalogLoading(false); });
    return () => { active = false; };
  }, []);
  ...
  const charge = () => {
    ...
    if (sale.some(line => !Number.isInteger(Number(line.product.id)))) return setMessage('Demo products cannot be sold. Start the backend and sync the live catalog.');
    ...
    api.post('/pos/sale', { cart: sale.map(line => ({ product: { id: line.product.id }, quantity: line.quantity, variant: {} })), payment_method: tender, cash_received: tender === 'cash' ? Number(cashReceived) : undefined, cashier_name: 'Cashier', checkout_key: checkoutKey })
      .then(({ data }) => { setMessage(`Sale ${data.order_id} completed`); setLastReceipt(data); setSale([]); setCashReceived(''); })
      .catch(error => setMessage(error.response?.data?.message || 'Sale could not be saved'));
  };

  const holdSale = () => { if (!sale.length) return; const draft = { sale, cashReceived, tender }; localStorage.setItem('nuvara_held_sale', JSON.stringify(draft)); setHeld(draft); setSale([]); setCashReceived(''); setMessage('Sale held for later'); };
  const resumeSale = () => { if (!held) return; setSale(held.sale || []); setCashReceived(held.cashReceived || ''); setTender(held.tender || 'cash'); localStorage.removeItem('nuvara_held_sale'); setHeld(null); };
  const openSession = async () => {
    try { await api.post('/pos/registers/open', { register_code: 'REGISTER-01', cashier_name: 'Cashier', opening_float: 0 }); setRegisterOpen(true); setMessage('Register session opened'); }
    catch (error) { if (error.response?.status === 409) setRegisterOpen(true); else setMessage('Could not open register session'); }
  };
  return <MotionConfig reducedMotion="user"><div className="pos-shell min-h-screen bg-bg-primary text-text-primary">
    <header className="pos-topbar h-16 px-4 lg:px-6 flex items-center justify-between border-b border-border bg-surface">
      ...
      <div className="hidden md:flex items-center gap-3 text-xs"><span className="pos-status-dot" /> Online <span className="text-border">|</span> Main store · Register 01 <button onClick={openSession} className="pos-session-btn">{registerOpen ? 'Session open' : 'Open session'}</button></div>
    ...
    <div className="pos-body flex">
      ...
      <main className="flex-1 p-4 lg:p-6 min-w-0"><div className="flex items-center justify-between mb-5"><div><p className="text-xs text-accent uppercase tracking-widest font-bold">Point of sale</p><h1 className="font-display text-3xl">New sale</h1></div><button className="lg:hidden pos-outline-btn" onClick={() => setMobileCart(true)}><ShoppingCart size={17}/> Sale ({sale.length})</button></div>
        ...
        {catalogLoading && <p className="text-xs text-text-secondary mb-3">Syncing live catalog…</p>}
  ...
  </div>{lastReceipt && <div className="pos-receipt-toast"><div><strong>Sale completed</strong><span>{lastReceipt.order_id}</span></div><button onClick={() => window.print()}>Print receipt</button><button onClick={() => setLastReceipt(null)} aria-label="Close receipt"><X size={16}/></button></div>}
  </div></MotionConfig>;
};
